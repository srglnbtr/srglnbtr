...
export default function AdminSettingsPage() {
      ...
      await setDoc(
        doc(db, COLLECTIONS.settings, DOC_IDS.settings),
        {
          socialLinks: {
            ...
            email: form.socialLinks.email.trim(),
          },
  ...
  return (
    <div>
      ...
      <p className="mt-1 text-sm text-slate-500">Сошиал холбоос, нийтлэг CV холбоос.</p>
      ...
      <GlassCard className="mt-6 space-y-4 p-6">
        {(
          [
            ...
            { key: "email" as const, label: "И-мэйл (mailto:...)" },
          ] as const
        ).map(({ key, label }) => (
          <div key={key}>
        ...
        <div>
          <label className="mb-1 block text-xs text-slate-400">CV URL (нийтлэг)</label>
          <input
            className="w-full rounded-xl border border-white/10 bg-[#050816]/70 px-3 py-2 text-sm"
            value={form.cvUrl}
